...
from skimage import io, measure, morphology, feature, filters
...
#     # load binary file
#     file_name = pic2d.name
#     pic2d_arr = io.imread(pic2d, as_gray=True).astype(bool)
#
#     # compute skeleton and distance
#     smooth_pic2d_arr = filters.gaussian(pic2d_arr, sigma=2)
#     plt.imsave(str(pic2d).replace(".png", "_smooth.png"), smooth_pic2d_arr)
#     skeleton, dist = morphology.medial_axis(smooth_pic2d_arr > 0.5, return_distance=True)
#
#     # compute distance transform
#     distance_transform = skeleton * dist
#
#     # save distance transform
#     np.save(pic2d.parent / Path(file_name.replace(".png", "_dt.npy")), distance_transform)


if __name__ == "__main__":
    for pic2d in Path("input_data/d0").glob("*.crop500.png"):
        generate_distance_transform_for_pic2d(pic2d)

    for dt in Path("input_data/d0").glob("*.npy"):
        plt.imsave(str(dt).replace(".npy", ".png"), np.load(dt))
