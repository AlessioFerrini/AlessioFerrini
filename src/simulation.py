...
def compute_c0(c_old: dolfinx.fem.Function,
    ...
    """
    Get the initial condition for the capillaries.

    :param spatial_dimension: 2D or 3D (default is 2D)
    :param c_old: FEniCS function to store initial condition
    :param egg_parameters: egg specific Dict
    :param mesh_parameters: mesh parameters (used in the computation)
    """
    # load binary image
    pic2d_file = egg_parameters["pic2d"]
    binary = skimage.io.imread(pic2d_file)
    c_old_expression = VesselReconstruction(binary_array=binary,
...
def compute_mesh(spatial_dimension,
    ...
    :param egg_parameters: egg parameters
    ...
    # load Lx and Ly form egg_parameters
    Lx_real: Quantity = float(egg_parameters["simulation_box"]["Lx"]["value"]) \
                             * local_ureg(egg_parameters["simulation_box"]["Lx"]["mu"])
    Ly_real: Quantity = float(egg_parameters["simulation_box"]["Ly"]["value"]) \
                             * local_ureg(egg_parameters["simulation_box"]["Ly"]["mu"])
    if spatial_dimension == 3:
        Lz_real: Quantity = float(egg_parameters["simulation_box"]["Lz"]["value"]) \
                                 * local_ureg(egg_parameters["simulation_box"]["Lz"]["mu"])
    else:
        Lz_real = 0. * local_ureg(egg_parameters["simulation_box"]["Lx"]["mu"])
...
class CAMSimulation:
    def __init__(self,
        ...
        :param sim_parameters: simulation parameters.
        :param egg_parameters: patient-specific parameters.
    ...
    def _generate_mesh(self):
        self.mesh, self.mesh_parameters = compute_mesh(self.spatial_dimension,
                                                       ...
                                                       self.egg_parameters,
                                                       self.ureg)
        self.mesh_bbt = dolfinx.geometry.bb_tree(self.mesh, self.mesh.topology.dim)
    ...
    def _get_c_locator(self):
        """
        Return a function to locate the part of the domain where there are capillaries
        :return:
        """
        def capillaries_locator(x):
            points_on_proc, cells = get_colliding_cells_for_points(x.T, self.mesh, self.mesh_bbt)
            c_values = np.ravel(self.c_old.eval(points_on_proc, cells))
            return c_values > 0.9
